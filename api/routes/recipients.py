# [DOC] Recipients API — the sender's address book that maps friendly nicknames to permanent IDX values.
# [DOC] Core privacy principle: users never type a recipient's session ID (which rotates every 24 h).
# [DOC] They add a recipient by IDX once; the system resolves IDX → current session ID on every payment.
...
# [DOC] Blueprint groups these routes under /api/recipients.
from flask import Blueprint, request, jsonify

# [DOC] require_auth validates JWT and injects current_user + db into each route.
from api.middleware.auth import require_auth
from api.middleware.rate_limiter import limiter, get_rate_limit
# [DOC] RecipientService encapsulates all DB logic for managing the recipients table.
...
def get_all_recipients(current_user, db):
    ...
    """
    try:
        # [DOC] Pass the DB session to the service layer (dependency injection pattern).
        service = RecipientService(db)
        # [DOC] get_user_recipients() queries recipients WHERE sender_idx = current_user.idx.
        recipients = service.get_user_recipients(current_user.idx)

        return jsonify({
            'success': True,
            # [DOC] to_dict() on each recipient excludes session_id — users only see IDX and nickname.
...
def add_recipient(current_user, db):
    ...
    """
    try:
        # [DOC] request.get_json() parses the JSON request body into a Python dict.
        data = request.get_json()

        # [DOC] Validate that both required fields are present before touching the DB.
        ...
        # [DOC] add_recipient() verifies the recipient IDX exists, creates the DB row,
        # [DOC] and sets added_at = now so the 30-minute timer can be enforced.
        service = RecipientService(db)
        ...
        response = {
            'success': True,
            'recipient': recipient.to_dict(),  # [DOC] Sessions are hidden inside to_dict().
        }

        # [DOC] can_transact() returns False if fewer than 30 minutes have elapsed since added_at.
        if not recipient.can_transact():
            # [DOC] time_until_can_transact() returns a timedelta; convert to whole minutes for the message.
        ...
        # [DOC] HTTP 201 Created — a new resource (recipient row) was created.
        return jsonify(response), 201

    except ValueError as e:
        # [DOC] ValueError comes from the service layer (e.g. duplicate IDX, IDX not found).
        return jsonify({
            'success': False,
            'error': str(e)
        }), 400
...
def get_recipient_by_nickname(current_user, db, nickname):
    ...
    try:
        service = RecipientService(db)
        # [DOC] Lookup is scoped to current_user.idx so user A cannot read user B's address book.
...
def update_nickname(current_user, db, recipient_id):
    ...
    try:
        ...
        service = RecipientService(db)
        # [DOC] update_nickname() finds the row by integer PK and changes the nickname column.
...
def remove_recipient(current_user, db, recipient_id):
    ...
    try:
        service = RecipientService(db)
        # [DOC] remove_recipient() deletes the row by PK; raises ValueError if not found.
        ...
        # [DOC] 404 is appropriate when the resource to delete doesn't exist.
        return jsonify({
            'success': False,
            'error': str(e)
        }), 404
