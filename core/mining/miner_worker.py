# [DOC] MinerWorker — one thread per user who opts in to competitive mining via the /api/mining/start endpoint.
# [DOC] Multiple MinerWorkers race to find a valid nonce; the first one to submit wins the block fee.
# [DOC] This is the per-user mining thread; compare with core/workers/mining_worker.py (the system-wide daemon).

"""
Miner Worker - Individual Mining Thread
Purpose: Continuously mine blocks for a single user

Features:
- Runs in dedicated thread per miner
- Fetches pending transactions
- Performs PoW mining
- Submits solutions to mining pool
- Tracks performance metrics

Flow:
1. Start worker thread
2. Check for pending transactions
3. Mine block (find valid nonce)
4. Submit solution to pool
5. If accepted: Winner! Get fees
6. If rejected: Too late, someone else won
7. Repeat
"""

import threading
import time
from typing import Optional
from datetime import datetime

from database.connection import SessionLocal
# [DOC] Transaction and TransactionStatus: used to check whether there are pending txs before mining.
from database.models.transaction import Transaction, TransactionStatus
# [DOC] MinerStatistics ORM model: stores cumulative metrics for this miner (blocks won/lost, hash rate).
from database.models.miner import MinerStatistics
# [DOC] MiningService: contains the actual PoW loop (SHA-256 nonce search).
from core.consensus.pow.miner import MiningService
from config.settings import settings


class MinerWorker:
    """
    Individual miner worker thread

    Each user who wants to mine gets their own MinerWorker.
    Workers compete to find valid blocks - first to find wins.

    Attributes:
        user_idx: Miner's IDX
        pool: Reference to mining pool
        running: Whether worker is active
        thread: Worker thread
    """

    def __init__(self, user_idx: str, pool):
        """
        Initialize miner worker

        Args:
            user_idx: User's IDX
            pool: Mining pool reference
        """
        # [DOC] user_idx identifies which user's balance gets the fee when this worker wins a block.
        self.user_idx = user_idx
        # [DOC] pool is the MiningPool that coordinates submissions and prevents duplicate block writes.
        self.pool = pool
        # [DOC] running controls the _mining_loop while condition.
        self.running = False
        self.thread = None
        # [DOC] hash_attempts tracks how many SHA-256 hashes this worker has tried (used in hash-rate calc).
        self.hash_attempts = 0
        # [DOC] start_time records when the worker started — used to compute elapsed time for hash rate.
        self.start_time = None

    def start(self):
        """Start mining worker thread"""
        if self.running:
            return

        self.running = True
        self.start_time = time.time()
        # [DOC] daemon=True: this thread won't prevent the process from exiting.
        self.thread = threading.Thread(target=self._mining_loop, daemon=True)
        self.thread.start()

        print(f"⛏️  Miner worker started: {self.user_idx[:16]}...")

    def stop(self):
        """Stop mining worker"""
        self.running = False
        if self.thread:
            # [DOC] timeout=2: don't wait more than 2 seconds — the thread may be in the middle of mining.
            self.thread.join(timeout=2)

        # [DOC] Write final hash_attempts and hash_rate to the DB before the thread exits.
        self._update_final_stats()

        print(f"⏹️  Miner worker stopped: {self.user_idx[:16]}...")

    def _mining_loop(self):
        """
        Main mining loop

        Continuously:
        1. Check for pending transactions
        2. Try to mine a block
        3. Submit if successful
        4. Repeat
        """
        # [DOC] Read timeout from settings (default 300 seconds); guards against infinitely long mining attempts.
        timeout = getattr(settings, 'MINING_TIMEOUT_SECONDS', 300)

        while self.running:
            try:
                success = self._mine_one_block(timeout=timeout)

                if success:
                    # [DOC] Won a block — loop immediately to look for the next one.
                    pass
                else:
                    # [DOC] Nothing to mine or lost the race — wait before retrying to avoid CPU spinning.
                    time.sleep(5)

            except Exception as e:
                print(f"❌ Mining error ({self.user_idx[:16]}...): {e}")
                # [DOC] Sleep longer on errors to avoid hammering the DB in a tight failure loop.
                time.sleep(10)

    def _mine_one_block(self, timeout: int = 300) -> bool:
        """
        Attempt to mine one block

        Args:
            timeout: Maximum seconds to spend mining

        Returns:
            True if block mined and submitted, False otherwise
        """
        db = SessionLocal()

        try:
            # [DOC] Quick count query — skip the expensive PoW if there's nothing to mine.
            pending_count = db.query(Transaction).filter(
                Transaction.status == TransactionStatus.PENDING
            ).count()

            if pending_count == 0:
                return False  # [DOC] No work available — caller will sleep and retry.

            # [DOC] Create a MiningService scoped to this user (so fee credit goes to this user).
            miner = MiningService(db, self.user_idx)

            attempt_start = time.time()

            # [DOC] mine_pending_transactions() runs PoW — this is the CPU-intensive step.
            # [DOC] Returns the mined BlockPublic row, or None if another thread already claimed the txs.
            block = miner.mine_pending_transactions(batch_size=10)

            if not block:
                return False

            mining_time = time.time() - attempt_start

            # [DOC] Submit to the pool — the pool uses a lock so only the first submission wins.
            accepted = self.pool.submit_solution(self.user_idx, block)

            if accepted:
                # [DOC] We were first — update our statistics with the winning time.
                self._update_stats_after_win(mining_time)
                print(f"🎉 {self.user_idx[:16]}... WON! Mined block #{block.block_index} in {mining_time:.2f}s")
                return True
            else:
                # [DOC] Another worker submitted first — this block is wasted effort.
                self.pool.report_late_solution(self.user_idx)
                print(f"⏰ {self.user_idx[:16]}... lost race (took {mining_time:.2f}s)")
                return False

        finally:
            db.close()

    def _update_stats_after_win(self, mining_time: float):
        """
        Update miner statistics after winning a block

        Args:
            mining_time: Time taken to mine this block (seconds)
        """
        db = SessionLocal()

        try:
            miner_stats = db.query(MinerStatistics).filter(
                MinerStatistics.user_idx == self.user_idx
            ).first()

            if not miner_stats:
                return

            # [DOC] Update the running average mining time using the standard incremental formula:
            # [DOC] new_avg = (old_avg * n + new_value) / (n + 1)
            if miner_stats.avg_mining_time_seconds is None:
                miner_stats.avg_mining_time_seconds = mining_time
            else:
                total_blocks = miner_stats.total_blocks_mined
                current_avg = miner_stats.avg_mining_time_seconds
                new_avg = (current_avg * total_blocks + mining_time) / (total_blocks + 1)
                miner_stats.avg_mining_time_seconds = new_avg

            db.commit()

        finally:
            db.close()

    def _update_final_stats(self):
        """Update statistics when miner stops"""
        # [DOC] start_time is None if start() was never called — guard against that.
        if not self.start_time:
            return

        db = SessionLocal()

        try:
            miner_stats = db.query(MinerStatistics).filter(
                MinerStatistics.user_idx == self.user_idx
            ).first()

            if not miner_stats:
                return

            # [DOC] Total wall-clock time this worker was running (may include idle sleep time).
            total_time = time.time() - self.start_time

            # [DOC] Accumulate hash attempts into the DB column (persists across worker restarts).
            miner_stats.total_hash_attempts += self.hash_attempts

            # [DOC] Hash rate = attempts per second; only meaningful if we actually hashed something.
            if total_time > 0 and self.hash_attempts > 0:
                hash_rate = self.hash_attempts / total_time
                miner_stats.hash_rate_per_second = hash_rate

            db.commit()

        finally:
            db.close()


# For testing
if __name__ == "__main__":
    print("=== Miner Worker Test ===\n")

    # Create a mock pool
    class MockPool:
        def submit_solution(self, miner_idx, block):
            print(f"📥 Solution submitted by {miner_idx[:16]}...")
            return True  # Accept

        def report_late_solution(self, miner_idx):
            print(f"⏰ Late solution from {miner_idx[:16]}...")

    pool = MockPool()

    # Create worker
    worker = MinerWorker("IDX_TEST_MINER", pool)
    worker.start()

    # Let it run for a bit
    time.sleep(10)

    # Stop worker
    worker.stop()

    print("\n✅ Miner worker test complete")
