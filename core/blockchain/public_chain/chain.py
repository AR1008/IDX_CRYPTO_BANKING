...
# [DOC] json is used in to_dict/from_dict for serialising and deserialising the full chain
import json
# [DOC] Type hints make the API explicit: List, Optional, Dict, Any describe argument and return types
from typing import List, Optional, Dict, Any
# [DOC] datetime is imported but not used directly in this module — available for callers or future use
from datetime import datetime
# [DOC] Block is the individual building unit of the chain — each Blockchain is a sequence of Block objects
from core.blockchain.public_chain.block import Block
# [DOC] settings.POW_DIFFICULTY is the default mining difficulty (number of leading hex zeros required)
from config.settings import settings
# [DOC] time.time() gives the current Unix timestamp — used when creating the genesis block
import time
...
class Blockchain:
    ...
    def __init__(self, difficulty: int = None):
        ...
        """
        # [DOC] self.chain holds the ordered list of Block objects — index 0 is genesis, index N is the latest
        self.chain: List[Block] = []
        # [DOC] Use the provided difficulty or fall back to the global setting; `or` works because 0 is falsy (difficulty 0 would be unsafe)
        self.difficulty = difficulty or settings.POW_DIFFICULTY

        # [DOC] Create the genesis block immediately — the chain is never in an empty state after __init__
    ...
    def _create_genesis_block(self) -> None:
        ...
        # [DOC] index=0 by convention identifies the genesis block; previous_hash="0" signals there is no parent block
        # [DOC] transactions=[] because the genesis block carries no real transaction data — it only anchors the chain
        genesis_block = Block(
            index=0,
            transactions=[],
            previous_hash="0",
        ...
        # [DOC] Mine the genesis block to satisfy the PoW difficulty — this proves computational work was done from the start
        # Mine genesis block
        genesis_block.mine_block(difficulty=self.difficulty)

        # [DOC] Append the mined genesis block to self.chain — it will always be at index 0
        # Add to chain
    ...
    def get_latest_block(self) -> Block:
        ...
        """
        # [DOC] self.chain[-1] returns the last element of the list — always the newest block appended
        return self.chain[-1]
    ...
    def add_block(self, transactions: List[str]) -> Block:
        ...
        Example:
            ...
            Block added: #1
        """
        # [DOC] Fetch the current chain tip so the new block can reference its hash as previous_hash
        # Get latest block to link to
        latest_block = self.get_latest_block()

        # [DOC] Create the new block with index = latest + 1 and previous_hash = latest block's hash (forming the chain link)
        # Create new block
        new_block = Block(
        ...
        # [DOC] mine_block() increments nonce until SHA-256(block) starts with 'difficulty' zeros — computationally expensive by design
        # Mine the block (find valid nonce)
        new_block.mine_block(difficulty=self.difficulty)

        # [DOC] Validate the mined block before accepting it — guards against any bug in mine_block() producing an invalid result
        # Validate before adding
        if not new_block.is_valid(difficulty=self.difficulty):
            raise ValueError("Invalid block - validation failed!")

        # [DOC] Verify chain continuity: the new block's previous_hash must exactly equal the current chain tip's hash
        ...
        # [DOC] Only append after all checks pass — the chain is always in a valid state
        # Add to chain
    ...
    def is_valid(self) -> bool:
        ...
        """
        # [DOC] Walk every block in order from genesis (index 0) to the latest
        # Check each block in the chain
        for i in range(len(self.chain)):
            current_block = self.chain[i]

            # [DOC] is_valid() on each block checks PoW, hash integrity, and timestamp sanity
            # Validate block itself
            if not current_block.is_valid(difficulty=self.difficulty):
                print(f"❌ Block #{i} is invalid!")
                return False

            # [DOC] Skip the linkage check for genesis (i=0) because it has no parent block
            # Check linkage (except genesis block)
            if i > 0:
                previous_block = self.chain[i - 1]

                # [DOC] If someone changes data in block i-1, its hash changes, breaking this comparison for block i
        ...
        # [DOC] All blocks individually valid and all links intact — the chain is trustworthy
        # All checks passed!
        return True
    ...
    def get_block_by_index(self, index: int) -> Optional[Block]:
        ...
        """
        # [DOC] Guard against negative or out-of-range indices — return None gracefully instead of raising IndexError
    ...
    def get_block_by_hash(self, block_hash: str) -> Optional[Block]:
        ...
        """
        # [DOC] Linear scan — acceptable for research prototype; production would use an index (dict or DB column)
    ...
    def get_length(self) -> int:
        ...
        Example:
            ...
            2  # Genesis + 1 block
        """
        # [DOC] len(self.chain) includes the genesis block — a fresh chain has length 1, not 0
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        """
        # [DOC] Sum the transaction count across all blocks by adding len(block.transactions) for each block
        ...
        return {
            ...
            # [DOC] self.chain[0].hash is always the genesis block hash — useful for verifying chain identity
            'genesis_hash': self.chain[0].hash,
            'latest_hash': self.get_latest_block().hash,
            'difficulty': self.difficulty,
            # [DOC] is_valid() runs the full chain validation here — may be slow for very long chains
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        """
        # [DOC] Serialise each Block to a dict using Block.to_dict() — the result is a JSON-safe nested structure
        return {
    ...
    def from_dict(data: Dict[str, Any]) -> 'Blockchain':
        ...
        """
        # [DOC] __new__ allocates a Blockchain object without calling __init__ — avoids creating a second genesis block
        blockchain = Blockchain.__new__(Blockchain)
        # [DOC] Restore the difficulty from the serialised data so validation uses the same target as when the chain was built
        blockchain.difficulty = data['difficulty']
        # [DOC] Reconstruct each Block from its dict representation using Block.from_dict()
        blockchain.chain = [
    ...
    def __repr__(self) -> str:
        ...
        # [DOC] __repr__ gives a one-line summary useful in REPL sessions and log output
        return (
