...
class NestedThresholdSharing:
    ...
    # [DOC] PRIME is a 256-bit prime number that defines the finite field for all polynomial arithmetic.
    # [DOC] Every share value, coefficient, and recovered secret is an integer in [0, PRIME).
    # Prime modulus (256-bit prime for finite field)
    PRIME = 2**256 - 189

    # [DOC] REGULATORY_AUTHORITIES maps each authority name to its integer x-coordinate in the inner Shamir polynomial.
    # [DOC] These IDs are the x-values at which the inner polynomial is evaluated to generate each authority's share.
    # [DOC] In the worldwide system these map to: FFA (Federal Financial Authority), FIU, FLEA, NTA.
    ...
    def _encode_secret(self, secret: str) -> int:
        ...
        """
        # [DOC] SHA-256 converts the secret string to a uniform 256-bit byte string.
        # [DOC] hexdigest() gives us 64 hex characters = 32 bytes = 256 bits.
        secret_hash = hashlib.sha256(secret.encode()).hexdigest()
        # [DOC] int(..., 16) parses the hex string as a base-16 integer.
        secret_int = int(secret_hash, 16)
        # [DOC] Reduce modulo PRIME to fit the integer inside the finite field.
    ...
    def _evaluate_polynomial(self, coefficients: List[int], x: int) -> int:
        ...
        Returns:
            int: P(x) mod PRIME
        """
        # [DOC] result will hold the running total of all polynomial terms evaluated at x.
        result = 0
        for i, coeff in enumerate(coefficients):
            # [DOC] pow(x, i, PRIME) is Python's built-in modular exponentiation: computes x^i mod PRIME fast.
            # [DOC] Multiply by the coefficient and add to the running total.
            result += coeff * pow(x, i, self.PRIME)
            # [DOC] Reduce after each addition to keep the number inside the field (avoids unbounded growth).
            result %= self.PRIME
        return result
    ...
    def _split_shamir(self, secret_int: int, threshold: int, num_shares: int) -> List[Dict[str, int]]:
        ...
        """
        # [DOC] Build a random polynomial of degree (threshold - 1).
        # [DOC] The constant term a0 = secret_int, so P(0) = secret.
        # Generate random polynomial: P(x) = secret + a1*x + a2*x^2 + ...
        coefficients = [secret_int]
        for _ in range(threshold - 1):
            # [DOC] Each higher-degree coefficient is independently random — information-theoretic security.
            # [DOC] Fewer than threshold shares cannot determine any coefficient including the secret.
            coefficients.append(secrets.randbelow(self.PRIME))

        # [DOC] shares will hold the list of (x, y) share dicts, one per authority.
        # Generate shares
        shares = []
        for i in range(1, num_shares + 1):
            # [DOC] Evaluate P(i) — the share value for the i-th holder.
            # [DOC] x-coordinates start at 1 (never 0, because P(0) = the secret itself).
    ...
    def _lagrange_interpolation(self, shares: List[Dict[str, int]], x: int = 0) -> int:
        ...
        """
        # [DOC] secret accumulates the sum of Lagrange basis terms — this IS the interpolation formula.
        secret = 0

        for i, share_i in enumerate(shares):
            # [DOC] xi is this share's x-coordinate (the holder's fixed ID or inner index).
            # [DOC] yi is the share value P(xi).
            xi = share_i['x']
            yi = share_i['y']

            # [DOC] Compute the Lagrange basis polynomial L_i(x) = product over j!=i of (x - xj)/(xi - xj).
            ...
            for j, share_j in enumerate(shares):
                if i != j:
                    xj = share_j['x']
                    # [DOC] Numerator factor: (x - xj) — evaluating at x=0 gives -xj.
                    numerator *= (x - xj)
                    # [DOC] Denominator factor: (xi - xj) — scales the basis so L_i(xi) = 1.
                    denominator *= (xi - xj)

            # [DOC] Modular inverse: in finite field arithmetic, a/b = a * b^(-1) mod PRIME.
            # [DOC] pow(a, -1, PRIME) is the modular inverse (Python 3.8+).
            # Modular inverse
            denominator_inv = pow(denominator % self.PRIME, -1, self.PRIME)

            # [DOC] Add the i-th Lagrange term: yi * L_i(x) = yi * (numerator / denominator).
            # Add term
    ...
    def split_secret(self, secret: str) -> Dict[str, Any]:
        ...
        """
        # [DOC] Convert the master secret string into a finite-field integer for polynomial arithmetic.
        # Encode secret
        secret_int = self._encode_secret(secret)

        # [DOC] OUTER LAYER: 2-of-2 Shamir splits the secret into exactly 2 shares.
        # [DOC] threshold=2 means BOTH shares are required — Company cannot decrypt alone, Court cannot decrypt alone.
        # [DOC] The two outer shares represent: (1) Company share, (2) Court_Combined (an intermediate value).
        ...
        # [DOC] outer_shares[0] is the Company share — x=1 on the outer polynomial.
        company_share = outer_shares[0]  # x=1
        # [DOC] outer_shares[1] is Court_Combined — x=2 on the outer polynomial.
        # [DOC] Court_Combined is NOT given to any single authority; instead, it is itself split in the inner layer.
        court_combined = outer_shares[1]  # x=2 (will be split further)

        # [DOC] INNER LAYER: 1-of-4 Shamir splits Court_Combined's VALUE into 4 regulatory authority shares.
        # [DOC] threshold=1 means ANY SINGLE regulatory authority can reconstruct Court_Combined.
        # [DOC] This is how "any one of FFA/FIU/FLEA/NTA" is implemented mathematically.
        ...
        # [DOC] Package the company share — marked mandatory so reconstruct_secret() can enforce its presence.
        # Package shares
        shares = {
            'company': {
                'type': 'company_share',
        ...
        # [DOC] Package each regulatory share — marked as inner-layer with its authority name and the court_combined_x.
        # [DOC] court_combined_x is needed at reconstruction time to place Court_Combined at the right x-coordinate on the outer polynomial.
    ...
    def reconstruct_secret(
        ...
        """
        # [DOC] Enforce that the Company share was actually supplied — None means access is denied.
        # Validate inputs
        if company_share is None:
            raise ValueError("Company share is mandatory and cannot be None")

        # [DOC] Enforce that exactly one regulatory share was provided.
        if regulatory_share is None:
            raise ValueError("Regulatory share is mandatory (need 1 of 4)")

        # [DOC] Validate the company_share is of the correct type to prevent mix-up with regulatory shares.
        if company_share.get('type') != 'company_share':
            raise ValueError(f"Invalid company share type: {company_share.get('type')}")

        # [DOC] Validate the regulatory_share type to prevent passing a company share in the wrong slot.
        if regulatory_share.get('type') != 'regulatory_share':
            raise ValueError(f"Invalid regulatory share type: {regulatory_share.get('type')}")

        # [DOC] INNER LAYER RECOVERY: because inner threshold=1, a single regulatory share IS Court_Combined directly.
        # [DOC] regulatory_share['y'] is the y-value on the 1-of-4 polynomial, which equals Court_Combined when threshold=1.
        ...
        # [DOC] court_combined_x is the x-coordinate Court_Combined had on the OUTER polynomial (stored at split time).
        court_combined_x = regulatory_share['court_combined_x']

        # [DOC] OUTER LAYER RECOVERY: assemble the two outer shares — Company (x=1, y=company_y) and Court_Combined (x=court_x, y=court_y).
        # [DOC] With both points, Lagrange interpolation on the degree-1 outer polynomial recovers P(0) = master secret.
        ...
        # [DOC] Lagrange interpolation at x=0 reconstructs the master secret integer from the two outer share points.
        reconstructed_int = self._lagrange_interpolation(outer_shares, x=0)

        # [DOC] Re-encode the original_secret and compare — only correct shares produce the expected integer.
        # Verify against original secret
        expected_int = self._encode_secret(original_secret)

        # [DOC] If they match, the reconstruction is correct and the plaintext secret is returned.
        if reconstructed_int == expected_int:
            return original_secret
        else:
            # [DOC] Mismatch: shares were invalid, tampered, or wrong original_secret was supplied.
    ...
    def verify_access_structure(self, shares: List[Dict[str, Any]]) -> Dict[str, bool]:
        ...
        """
        # [DOC] Flags to track which mandatory/optional slots are filled.
        has_company = False
        regulatory_authorities = []

        for share in shares:
            if share.get('type') == 'company_share':
                # [DOC] The company share slot is filled.
                has_company = True
            elif share.get('type') == 'regulatory_share':
                # [DOC] Record which regulatory authority contributed — useful for audit logs.
                regulatory_authorities.append(share.get('authority'))

        # [DOC] has_regulatory is True if at least one regulatory authority has provided a share.
        has_regulatory = len(regulatory_authorities) > 0
        # [DOC] can_decrypt is True only if BOTH the company AND at least one regulatory authority are present.
