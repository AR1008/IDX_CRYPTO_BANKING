...
class SessionIDGenerator:
    ...
    # [DOC] SESSION_DURATION_HOURS controls how long each session ID is valid before it rotates.
    # [DOC] Loaded from settings so it can be changed without modifying code (default: 24 hours).
    # [DOC] Rotation ensures that transactions from different days cannot be linked to the same account by an observer.
    ...
    def generate(idx: str, bank_name: str,
        ...
        # [DOC] Record the current time in milliseconds since Unix epoch (Jan 1 1970).
        # [DOC] Millisecond precision means two sessions generated 1ms apart will always differ.
        ...
        # [DOC] A cryptographically secure random salt adds 256 bits of entropy to each session ID.
        # [DOC] Even if timestamp_ms is the same (two calls in the same millisecond), the random salt keeps them distinct.
        # [DOC] secrets.token_bytes() uses the OS CSPRNG (e.g. /dev/urandom) — not guessable.
        ...
        if custom_salt is None:
            ...
            # [DOC] custom_salt is only used in tests where a deterministic session ID is needed for assertions.
            salt = custom_salt  # For testing with deterministic values

        # [DOC] .hex() converts the 32 raw bytes into a 64-character hex string safe for embedding in text.
        salt_hex = salt.hex()  # Convert to hexadecimal string

        # [DOC] Combine all components into a single string before hashing.
        # [DOC] The colon (':') delimiters prevent collisions between components (e.g. "AB:CD" != "A:BCD").
        ...
        # [DOC] SHA-256 converts the combined string into a fixed 32-byte (256-bit) digest.
        # [DOC] The resulting 64-character hex string is unguessable and reveals nothing about IDX or bank_name.
        # Step 4: Hash with SHA-256 (one-way function)
        # SHA-256 produces 256 bits = 32 bytes = 64 hex characters
        hash_bytes = hashlib.sha256(combined.encode('utf-8')).digest()
        hash_hex = hash_bytes.hex()

        # [DOC] Add "SESSION_" prefix so session IDs are visually distinct from IDXes and other identifiers in logs.
        # [DOC] Total format: SESSION_{64 hex chars} — total length 72 characters.
        # Step 5: Add SESSION_ prefix
        session_id = f"SESSION_{hash_hex}"

        # [DOC] Calculate the absolute timestamp when this session expires.
        # [DOC] The background worker compares datetime.now() against this value to decide when to rotate.
        ...
        # [DOC] Return both the session ID string and the expiry datetime so the caller can store both.
    ...
    def is_expired(expiry_datetime: datetime) -> bool:
        ...
        # [DOC] Compare current time against the stored expiry datetime.
        # [DOC] Returns True if the current moment is past the expiry — meaning the session must be rotated.
    ...
    def time_until_expiry(expiry_datetime: datetime) -> timedelta:
        ...
        # [DOC] Subtracting now from expiry gives a timedelta: positive = time remaining, negative = already expired.
    ...
    def format_expiry(expiry_datetime: datetime) -> str:
        ...
        if SessionIDGenerator.is_expired(expiry_datetime):
            # [DOC] If already expired, compute how long ago it expired for display in error messages.
            time_since = datetime.now() - expiry_datetime
            hours = int(time_since.total_seconds() // 3600)
            minutes = int((time_since.total_seconds() % 3600) // 60)
            return f"Expired {hours} hours, {minutes} minutes ago"
        else:
            # [DOC] If still valid, show how much time remains — useful for debugging and user-facing status endpoints.
