...
# [DOC] Transaction/TransactionStatus: ORM model for individual transfers and their state enum
from database.models.transaction import Transaction, TransactionStatus
# [DOC] TransactionBatch/BatchStatus: ORM model for groups of 100 txns and their state enum
from database.models.transaction_batch import TransactionBatch, BatchStatus
# [DOC] BankVotingRecord: ORM model — one row per bank per batch recording its APPROVE/REJECT vote
from database.models.bank_voting_record import BankVotingRecord
# [DOC] Bank: ORM model — tracks stake_amount, penalty_count, honest/malicious verifications
from database.models.bank import Bank
# [DOC] Treasury: ORM model — immutable ledger of SLASH and REWARD entries
from database.models.treasury import Treasury
# [DOC] BankAccount: ORM model — needed to validate sender/receiver existence in validate_transaction
from database.models.bank_account import BankAccount
# [DOC] Decimal: exact arithmetic for stake amounts and slash calculations
from decimal import Decimal
# [DOC] datetime/timezone: timestamps for vote verification and treasury entries
from datetime import datetime, timezone
# [DOC] List/Optional/Dict/Any: type hints for method signatures
from typing import List, Optional, Dict, Any
# [DOC] random: random.sample() selects 10% of batches for spot-check verification
import random
# [DOC] json: not directly used here but imported for potential future metadata serialization
import json
# [DOC] joinedload/selectinload: SQLAlchemy eager-loading strategies (imported, used selectively)
from sqlalchemy.orm import joinedload, selectinload


# [DOC] RBIValidator: implements the neutral third-party spot-check and slashing mechanism
class RBIValidator:
    """
    ...
    # [DOC] RANDOM_VERIFICATION_PERCENTAGE: the fraction of all batches RBI audits each cycle
    # Configuration
    RANDOM_VERIFICATION_PERCENTAGE = 10  # Re-verify 10% of batches

    # [DOC] Slashing penalty constants — escalate with each repeated offense to deter recidivism
    # Slashing penalties (escalating)
    ...
    # [DOC] DEACTIVATION_THRESHOLD: if stake drops below 30% of initial, the bank is removed
    ...
    def __init__(self, db=None):
        ...
        """
        # [DOC] Accept injected session (for tests/shared transactions) or create a fresh one
    ...
    def get_fiscal_year(self) -> str:
        ...
        """
        # [DOC] Indian fiscal year: April 1 to March 31
        now = datetime.now()
        if now.month >= 4:  # April onwards
            return f"{now.year}-{now.year + 1}"
        else:  # Jan-March
            # [DOC] January–March belongs to the prior year's fiscal year
    ...
    def select_batches_for_verification(
        ...
        """
        # [DOC] Fetch all batches in the given state (MINING = consensus approved, not yet mined)
        ...
        # [DOC] sample_size = ceil(10% of total); min 1 so at least one batch is always checked
        # Select 10% randomly
        sample_size = max(1, int(len(all_batches) * self.RANDOM_VERIFICATION_PERCENTAGE / 100))
        # [DOC] random.sample: select without replacement (no batch is audited twice in one cycle)
    ...
    def validate_transaction(self, tx: Transaction, sender_account: BankAccount = None, receiver_account: BankAccount = None) -> bool:
        ...
        """
        # [DOC] Only query the DB if accounts weren't pre-loaded by the caller (N+1 prevention)
        ...
        # [DOC] Both accounts must exist for the transaction to be structurally valid
        # Basic checks
        if not sender_account or not receiver_account:
            return False

        # [DOC] Amount must be strictly positive — zero or negative amounts are invalid
        if tx.amount <= 0:
            return False

        # [DOC] Fee cannot be negative (malformed transaction)
        if tx.fee < 0:
            return False
    ...
    def validate_batch(self, batch: TransactionBatch) -> bool:
        ...
        """
        # [DOC] Load all transactions in the batch in a single query
        ...
        # [DOC] An empty batch is considered invalid — there must be at least one transaction
        if not transactions:
            return False

        # [DOC] Collect every unique account ID referenced across all transactions in this batch
        ...
        # [DOC] Bulk-load all referenced accounts in ONE SQL query using IN clause
        ...
        if account_ids:
            ...
            # [DOC] Build a dict keyed by account.id for O(1) lookups inside the loop below
            accounts_dict = {acc.id: acc for acc in accounts}

        # [DOC] Validate each transaction using the pre-loaded account dict (no extra DB hits)
    ...
    def get_slash_percentage(self, bank: Bank) -> Decimal:
        ...
        """
        # [DOC] offense_count = current penalty_count + 1 (this is the next offense)
        offense_count = bank.penalty_count + 1

        # [DOC] 1st offense → 5%, 2nd → 10%, 3rd or more → 20%
        if offense_count == 1:
    ...
    def slash_bank(
        ...
        """
        # [DOC] Determine the percentage to slash based on how many times this bank has offended
        # Calculate slash amount
        slash_percentage = self.get_slash_percentage(bank)
        slash_amount = bank.stake_amount * slash_percentage

        # [DOC] Reduce the bank's on-chain stake and increment its offense counters
        # Update bank
        ...
        # [DOC] Deactivate the bank if its remaining stake falls below 30% of its initial stake
        ...
        # [DOC] Create an immutable Treasury SLASH entry — funds held until fiscal year end
        # Record in treasury
        ...
        # [DOC] Commit immediately so the slash is durable even if the caller crashes
        self.db.commit()
    ...
    def verify_batch_votes(
        ...
        """
        # [DOC] Load all votes cast for this batch by consortium banks
        ...
        # [DOC] Bulk-load all banks referenced in the votes — avoids N+1 inside the loop
        ...
        # [DOC] Dict keyed by bank_code for O(1) lookups in the vote loop below
        ...
        for vote in votes:
            # [DOC] A vote is correct if APPROVE on a valid batch OR REJECT on an invalid batch
            # Determine if vote was correct
            vote_correct = (vote.vote == 'APPROVE' and is_valid) or \
                          (vote.vote == 'REJECT' and not is_valid)

            # [DOC] Stamp the vote record with the RBI verdict and verification timestamp
            # Update vote record
            vote.is_correct = vote_correct
            vote.rbi_verified = True
            vote.rbi_verification_time = datetime.now(timezone.utc)

            # [DOC] Retrieve pre-loaded bank — no additional DB query needed
            ...
            if vote_correct:
                # [DOC] Increment honest_verifications — used in fiscal year reward allocation
                # Reward honest behavior
                bank.honest_verifications += 1
                correct_count += 1
                print(f"      ✅ {vote.bank_code}: {vote.vote} (correct)")
            else:
                # [DOC] APPROVE on an invalid batch = active fraud → slash
                ...
                if vote.vote == 'APPROVE' and not is_valid:
                    ...
                    # [DOC] Record slashing details on the vote row for the audit trail
                    # Update vote record
                    ...
                    # [DOC] REJECT on a valid batch = incorrect but not fraudulent → track but don't slash
        ...
        # [DOC] Commit all vote record updates and bank counter changes in one transaction
        self.db.commit()
    ...
    def verify_batch(self, batch: TransactionBatch):
        ...
        # [DOC] Step 1: RBI independently determines whether the batch is valid or invalid
        # Step 1: Validate batch
        is_valid = self.validate_batch(batch)

        # [DOC] Step 2: Cross-check each bank's stored vote against the RBI verdict; slash offenders
    ...
    def verify_random_batches(self):
        ...
        # [DOC] select_batches_for_verification returns a random 10% sample of MINING batches
        # Select batches
        ...
        # [DOC] Verify each selected batch sequentially — each call may slash banks
        # Verify each batch
    ...
    def process_bank_challenge(
        ...
        # [DOC] Load the challenged batch; abort early if it doesn't exist
        # Get batch
        ...
        # [DOC] Stamp every vote record for this batch with the challenger's identity and time
        ...
        for vote in votes:
            # [DOC] Only set challenged_by if not already challenged — first challenger wins
        ...
        # [DOC] Run the full RBI verification on the challenged batch — same logic as random spot-check
    ...
    def get_treasury_balance(self, fiscal_year: Optional[str] = None) -> Dict[str, Decimal]:
        ...
        """
        # [DOC] Default to the current fiscal year if none is specified
        if not fiscal_year:
            fiscal_year = self.get_fiscal_year()

        # [DOC] func.sum: SQL aggregate — totals all SLASH or REWARD entries for the year
        from sqlalchemy import func

        # [DOC] Sum all confiscated funds (SLASH) for the fiscal year
        total_slashed = self.db.query(func.sum(Treasury.amount)).filter(
            Treasury.entry_type == 'SLASH',
            Treasury.fiscal_year == fiscal_year
        ).scalar() or Decimal('0.00')

        # [DOC] Sum all already-paid rewards (REWARD) so balance reflects what is still available
        total_rewarded = self.db.query(func.sum(Treasury.amount)).filter(
            Treasury.entry_type == 'REWARD',
            Treasury.fiscal_year == fiscal_year
        ).scalar() or Decimal('0.00')

        # [DOC] balance = slashed − rewarded = funds still available for end-of-year distribution
    ...
    def close(self):
        ...
        # [DOC] Return the DB connection to the pool — important for long-running processes
        if self.db:
            self.db.close()
