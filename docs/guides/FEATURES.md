...
**How It Works**:
- N-bank consortium (configurable; default N=12)
- T-of-N consensus threshold (T = N-X, X < N/3; default T=10, 83%)
- Up to X banks can be malicious/crashed/unreachable
- T honest banks still reach consensus
...
- When T votes reached, threshold triggers
...
2. Request broadcast to all N banks
3. Each bank reviews independently
4. Banks vote: T approve, X reject (example)
5. T-th approval triggers threshold
6. Account automatically frozen
7. All banks notified

**System properties**:
- **Decentralization**: Distributed vote counting across nodes
- **Byzantine tolerance**: Tolerates up to X = N − T malicious banks (X < N/3)
...
**Technical Details**:
- Threshold: T-of-N (T = N-X, X < N/3; default T=10, N=12, 83%)
...
**How It Works**:
- All N banks form a "ring"
- When Bank 5 votes, creates ring signature
- Signature proves "one of these N banks voted"
...
Bank 5 creates ring signature:
  Signature: 0x9a7f...
  Ring: [Bank1, Bank2, ..., BankN]
  Vote: Approve

Verification:
  ✓ Signature valid for this ring
  ✓ One of N banks voted
...
- T-of-N threshold for block approval (default T=10, N=12, X=2)
- Instant finality (no probabilistic confirmation)
- Survives up to X malicious banks (X < N/3)
...
- Consensus threshold: T-of-N (T = N-X, X < N/3; default T=10, N=12, 83%)
