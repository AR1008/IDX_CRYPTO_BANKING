...
# [DOC] One row = one bank's vote on one batch; the system stores N rows per batch (one per consortium bank)
class BankVotingRecord(Base):
    ...
    # [DOC] Maps this Python class to the 'bank_voting_records' PostgreSQL table
    ...
    # Batch information
    # [DOC] Foreign reference to transaction_batches.batch_id — identifies which batch was voted on
    batch_id = Column(
        String(50),
        nullable=False,
        index=True,
    ...
    # Bank information
    # [DOC] Short bank code matching consortium_banks.bank_code, e.g. "HDFC"
    # [DOC] Note: the BBS04 group_signature field is anonymous — bank_code here is the SELF-REPORTED identity
    # [DOC] The opening authority can verify bank_code against the signature only under court order
    bank_code = Column(
        String(20),
        nullable=False,
        index=True,
    ...
    # Vote details
    # [DOC] "APPROVE" means the bank found all transactions in the batch valid; "REJECT" means at least one failed
    vote = Column(
        String(10),
        nullable=False,
    ...
    # Validation metrics
    # [DOC] Wall-clock milliseconds this bank took to validate all 100 transactions in the batch
    # [DOC] Used for performance monitoring; expected ~10-17ms per bank (range proof verify ~2.11ms × 100)
    ...
    # [DOC] Set after the regulatory authority spot-checks this batch; True = bank voted correctly, False = bank lied
    # [DOC] NULL until the re-verification pass runs (not set at voting time)
    is_correct = Column(
        Boolean,
        nullable=True,
    ...
    # [DOC] True once the regulatory authority has run its independent re-verification on this vote
    rbi_verified = Column(
        Boolean,
        nullable=False,
        default=False,
    ...
    # [DOC] Timestamp of when the regulatory authority marked this vote as verified
    ...
    # [DOC] True if this bank was penalized for an incorrect vote; stake_amount in consortium_banks was reduced
    was_slashed = Column(
        Boolean,
        nullable=False,
        default=False,
    ...
    # [DOC] Rupee amount removed from this bank's stake as punishment; also recorded in the treasury table
    slash_amount = Column(
        BigInteger,
        nullable=True,
    ...
    # [DOC] JSON-serialized BBS04 group signature — the bank's cryptographic proof of its vote
    # [DOC] Signature is anonymous: anyone can verify it came from SOME consortium bank, but cannot identify WHICH bank
    # [DOC] The opening authority (regulatory body) holds the opening key and can de-anonymize under court order only
    ...
    # [DOC] bank_code of the consortium member that formally disputed this vote; NULL if unchallenged
    ...
    # [DOC] When the challenge was filed; triggers a re-verification cycle by the regulatory authority
    ...
    # Timestamps
    # [DOC] Set automatically by the DB when the bank submits its vote; immutable after insertion
    created_at = Column(
        DateTime(timezone=True),
    ...
    # Indexes
    # [DOC] Six indexes: by batch, by bank, by correctness, by slash status, by verification flag, and composite batch+bank
    __table_args__ = (
