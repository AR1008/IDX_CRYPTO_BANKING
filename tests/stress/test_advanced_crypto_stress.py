...
# [DOC] Add project root to path so core/ and database/ modules are importable.
import sys
import os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../..")))

# [DOC] time: used to measure wall-clock elapsed time for each stress test section.
import time
from decimal import Decimal
from datetime import datetime

# [DOC] SessionLocal: creates the DB session shared across all stress test operations.
from database.connection import SessionLocal
from database.models.bank_account import BankAccount
# [DOC] Session: the 24-hour rotating pseudonym record; stress tests need active sessions to send transactions.
from database.models.session import Session
# [DOC] TransactionServiceV2: the main pipeline being stress-tested — commitment, range proof, nullifier, routing.
...
# [DOC] Open a single DB session for the entire stress test run — closed in the finally block at the end.
db = SessionLocal()
service = TransactionServiceV2(db)

# [DOC] Pull existing accounts and active sessions — stress tests use whatever data is already in the DB.
accounts = db.query(BankAccount).limit(20).all()
sessions = db.query(Session).filter(Session.is_active == True).limit(20).all()

# [DOC] session_map: keyed by (user_idx, bank_name) for O(1) lookup when building test transactions.
session_map = {}
