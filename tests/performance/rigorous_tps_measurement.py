...
# [DOC] Add project root to path so all core/ packages are importable.
import sys
import os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))

# [DOC] time: wall-clock measurements for each benchmark section.
import time
# [DOC] statistics: computes mean, stdev, and confidence intervals for the n=10 trial results.
import statistics
import secrets
...
# [DOC] MerkleTree: real SHA-256 binary tree — the actual implementation used in production batch processing.
from core.crypto.merkle_tree import MerkleTree
# [DOC] CommitmentScheme + RangeProof: legacy SHA-256 simulations included for baseline comparison only.
# [DOC] Note: production uses pedersen.py + bulletproofs_wrapper.py — these are NOT the real crypto paths.
from core.crypto.commitment_scheme import CommitmentScheme
from core.crypto.range_proof import RangeProof
# [DOC] BatchProcessor: provides BATCH_SIZE and CONSENSUS_THRESHOLD constants for the capacity calculations.
from core.services.batch_processor import BatchProcessor


# [DOC] RigorousTPSMeasurement: runs n=10 trials at each load level, computes 95% confidence intervals.
# [DOC] Designed for academic publication — adversarial methodology, not a simple pass/fail test.
