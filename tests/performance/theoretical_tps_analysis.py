...
# [DOC] Add the project root to the path so imports from core/ work.
import sys
import os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))

# [DOC] BatchProcessor holds the canonical constants: BATCH_SIZE=100, CONSENSUS_THRESHOLD=10, TOTAL_BANKS=12.
# [DOC] Importing it here ensures the analysis always reflects the actual deployed configuration, not hardcoded guesses.
...
# [DOC] Merkle time: measured experimentally from test_a_star_conference_level.py; 7-level SHA-256 tree over 100 leaves.
...
# [DOC] Consensus latency: conservative 10ms — banks vote in parallel so only the slowest vote counts.
...
# [DOC] DB write time: bulk insert of 100 transaction rows in one PostgreSQL statement.
...
# [DOC] Theoretical ceiling: assumes 100% CPU, zero API overhead, all banks always respond within the latency budget.
...
# [DOC] 30% overhead deduction for API routing, JSON serialization, queue management, and DB connection pooling.
...
# [DOC] Further 50% haircut for production: real network jitter, mixed workloads, peak-hour DB contention.
