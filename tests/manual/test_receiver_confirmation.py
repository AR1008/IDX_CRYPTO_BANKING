...
# [DOC] requests: sends HTTP calls to the running Flask API (must be started separately on port 5000 before running this test).
import requests
import json
import sys
# [DOC] random: generates unique PAN suffixes so this test can run multiple times without PAN collision in the DB.
import random
# [DOC] BASE_URL: the local Flask API endpoint. This is a manual end-to-end test — not a unit test.
...
# [DOC] register_user: calls POST /api/auth/register to create a user with a PAN card and an initial balance.
...
# [DOC] login_user: calls POST /api/auth/login, returns a JWT for use in all subsequent Authorization: Bearer headers.
...
# [DOC] create_transaction: calls POST /api/transactions/send — creates a Pedersen commitment + range proof, status → AWAITING_RECEIVER.
...
# [DOC] confirm_transaction: receiver calls POST /api/transactions/{hash}/confirm, picks which bank account to receive into.
# [DOC] After confirmation the status moves to PENDING, making the transaction eligible for batch collection and consensus.
