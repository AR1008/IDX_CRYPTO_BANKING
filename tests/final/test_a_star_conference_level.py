...
# [DOC] sys and os insert the project root into sys.path so all core crypto modules are importable
import sys
import os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))

# [DOC] unittest provides the structured test runner and assertion framework for conference-level reporting
import unittest
# [DOC] time measures elapsed wall-clock duration for IDX generation speed and attack simulation
import time
# [DOC] statistics computes mean, median, and quantiles used to characterise latency distributions
import statistics
# [DOC] hashlib provides SHA-256 used in IDX generation and entropy analysis
import hashlib
# [DOC] secrets provides cryptographically secure randomness for generating test inputs and attack samples
import secrets
from decimal import Decimal
from datetime import datetime, timedelta
from typing import List, Dict, Tuple
# [DOC] json serialises intermediate results for structured reporting
import json

# [DOC] Import all crypto modules needed across the six test categories below
...
# [DOC] AStarConferenceLevelTests wraps all adversarial tests as unittest.TestCase methods
# [DOC] so they can be run individually or as a full suite with verbosity=2 for conference review
class AStarConferenceLevelTests(unittest.TestCase):
    ...
    # [DOC] setUp initialises the results dict that accumulates findings across each test method;
    # [DOC] it is called fresh before every test so tests are independent
    def setUp(self):
    ...
    def test_01_idx_collision_resistance(self):
        ...
        # [DOC] sample_size=1,000,000 gives statistical coverage while remaining feasible in minutes;
        # [DOC] expected birthday collision probability is ~n^2/(2*2^256) which is negligibly small
        ...
        for i in range(sample_size):
            # [DOC] Generate random PAN (10-char alphanumeric) and RBI (6-char alphanumeric)
            # [DOC] to simulate the real IDX input space and check for SHA-256 collisions
            ...
            try:
                ...
                # [DOC] Invariant: every IDX value must be unique; any collision is a critical vulnerability
                if idx in idx_set:
                    collisions += 1
        ...
        # [DOC] expected_collisions uses the birthday paradox formula P ≈ n^2/(2*N) where N=2^256
        ...
        # [DOC] Assertion: zero collisions in 1M samples proves collision resistance at the required security level
    ...
    def test_02_session_linkability_attack(self):
        ...
        # [DOC] 100 users × 10,000 sessions simulates a realistic adversary who has observed
        # [DOC] a large volume of session IDs and tries to group them by user
        num_users = 100
        ...
        for user_id in range(num_users):
            # [DOC] Each user gets a random 256-bit IDX; sessions are generated via SessionIDGenerator
        ...
        # [DOC] Attack 1: check for repeated session token prefixes that would leak user identity
        ...
        # [DOC] Attack 2: Shannon entropy analysis — low entropy means session tokens are predictable
        ...
        # [DOC] Attack 3: random-guessing linkability test — an ideal privacy scheme should give exactly 50%
        ...
        for _ in range(num_link_attempts):
            ...
            # [DOC] The adversary guesses randomly because sessions are cryptographically unlinkable;
            # [DOC] any advantage above 50% means the session scheme leaks user identity
        ...
        # [DOC] Assertion: linkability advantage must be < 5% above random; larger means sessions are linkable
    ...
    def test_03_range_proof_soundness_attack(self):
        ...
        # [DOC] RangeProof is the legacy module used here to test its soundness property:
        # [DOC] no PPT adversary can produce a valid proof for an out-of-range value
        prover = RangeProof()
        ...
        for i in range(attack_attempts):
            # [DOC] The attacker tries to pass attack_amount=5000 (5× balance) as valid;
            # [DOC] soundness requires that verify_proof rejects any proof for a value outside [0, max_value]
            ...
            try:
                ...
                # [DOC] Invariant: verify_proof must return False for any proof of an out-of-range value
                if is_valid:
        ...
        # [DOC] soundness_error = fraction of forgery attempts that succeeded;
        # [DOC] must be essentially zero (< 2^-128 theoretically for Fiat-Shamir with 256-bit challenge)
        ...
        # [DOC] Assertion: zero forgeries in 100,000 attempts confirms soundness under empirical testing
    ...
    def test_04_byzantine_consensus_attack(self):
        ...
        # [DOC] total_banks=12 and consensus_threshold=8 match the default N=12, T=8 production config
        total_banks = 12
        ...
        # [DOC] Sweep from 1 to 12 malicious banks to identify safety, liveness, and censorship thresholds
        ...
        for num_malicious in range(1, total_banks + 1):
            ...
            # [DOC] malicious_can_approve: True if malicious banks alone can reach threshold (safety violation)
            ...
            # [DOC] honest_can_approve: True if honest banks can still reach threshold (liveness preserved)
            ...
            # [DOC] malicious_can_censor: True if malicious banks can prevent honest banks from reaching threshold
        ...
        # [DOC] Find the exact number of malicious banks that first breaks each security property
        ...
        # [DOC] Economic cost to launch a safety attack: must control 8 banks each with ₹1 crore stake
        ...
        # Verdict
        print(f"\n✅ SECURITY PROPERTIES:")
        print(f"  ✅ Safety: Secure against <67% malicious (theoretical BFT maximum)")
        ...
        # [DOC] Assertion: safety must not be compromised until at least consensus_threshold banks are malicious
        # Test assertions
    ...
    def test_05_threshold_sharing_attack(self):
        ...
        # [DOC] NestedThresholdSharing implements the RCTD primitive:
        # [DOC] outer 2-of-2 (Company + Court_Combined) + inner 1-of-4 (any one regulatory authority)
        ...
        # [DOC] split_secret produces shares for: company, rbi, fiu, cbi, income_tax
        # Split secret using nested threshold
        shares = tss.split_secret(secret)

        print(f"  Generated {len(shares)} shares: {list(shares.keys())}")

        # [DOC] Attack 1: attempt reconstruction with only a regulatory share (no Company);
        # [DOC] the RCTD outer 2-of-2 layer must make this cryptographically impossible
        ...
        for reg_auth in regulatory_authorities:
            ...
            try:
                ...
                # [DOC] Invariant: reconstruct_secret must raise ValueError with "Company share is mandatory"
                # [DOC] when the company share is absent — this is the RCTD mandatory-key enforcement
        ...
        # [DOC] Attack 2: even providing two regulatory shares (RBI + FIU) must still fail without Company
        ...
        # [DOC] Verification: Company + any one regulatory share must decrypt the secret correctly
        ...
        for reg_auth in regulatory_authorities:
            ...
            try:
                ...
                # [DOC] Invariant: valid combinations must recover the exact original secret string
                if recovered == secret:
    ...
    def test_06_performance_breaking_points(self):
        ...
        # [DOC] load_levels span three orders of magnitude to locate the latency cliff
        ...
        for target_tps in load_levels:
            ...
            # [DOC] Cap at 10k transactions per load level to bound test duration; target_tps sets the rate
            ...
            # [DOC] CommitmentScheme is used here to measure the cost of the commitment step per transaction
            ...
            for i in range(num_transactions):
                ...
                try:
                    # [DOC] create_commitment is the per-transaction crypto work; measures raw throughput
            ...
            # [DOC] p95 and p99 latency identify tail-latency spikes that would degrade user experience
            ...
            # [DOC] Status thresholds: BREAKING POINT is P99 > 1s, DEGRADED is avg > 100ms
            # Determine status
            if error_rate > 0.05:
        ...
        # [DOC] breaking_point records the first load level where status transitions to BREAKING POINT or FAILURE
        # Find breaking point
        breaking_point = None
        for load, result in results.items():
        ...
        # [DOC] Assertion: average latency at 4000 TPS target load must stay below 100ms
