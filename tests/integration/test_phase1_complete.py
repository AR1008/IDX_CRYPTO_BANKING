...
# [DOC] Add project root to path so all core/ and database/ packages are importable.
import sys
import os
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../..')))

# [DOC] time: used to measure mining duration and sleep for inter-step delays in the flow.
import time
import json
from decimal import Decimal
from datetime import datetime, timedelta

# [DOC] SessionLocal, engine, Base: DB connectivity and schema creation before each test run.
from database.connection import SessionLocal, engine, Base
from database.models.user import User
from database.models.transaction import Transaction, TransactionStatus
from database.models.block import BlockPublic
# [DOC] Aliased as UserSession to avoid collision with Python's built-in Session concept.
from database.models.session import Session as UserSession
from database.models.bank import Bank

# [DOC] TransactionService (v1 — legacy): the original transaction pipeline without real EC crypto.
# [DOC] InsufficientBalanceError, InvalidSessionError, UserNotFoundError: raised for invalid inputs.
...
# [DOC] MiningService: runs PoW SHA-256 with difficulty 4 to produce a new public block.
