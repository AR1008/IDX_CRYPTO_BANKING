...
# [DOC] requests: makes HTTP calls to the running Flask API. This is a live end-to-end test — the server must be running.
import requests
import json

# [DOC] BASE_URL: the local API server. Run `python3 -m api.app` before executing this test.
...
# [DOC] main: runs the complete travel account flow — create, view, close — verifying forex conversion and fee deduction end-to-end.
def main():
    print_header("✈️ PHASE 5: TRAVEL ACCOUNTS + FOREX TEST")

    # [DOC] Login first to get a JWT token; all subsequent requests are authenticated with this bearer token.
