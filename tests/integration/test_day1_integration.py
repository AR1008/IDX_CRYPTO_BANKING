...
# [DOC] Add project root to Python path so core/ and database/ are importable.
import sys
import os
...
# [DOC] SessionLocal: opens a DB connection; engine + Base used to create tables at test start.
from database.connection import SessionLocal, engine, Base
from database.models.user import User
from database.models.transaction import Transaction, TransactionStatus
# [DOC] BlockPublic: the publicly visible block (session IDs, commitments, hashes).
# [DOC] BlockPrivate: the encrypted block (real identities, amounts) decryptable only via court order.
...
# [DOC] SessionIDGenerator: creates 24-hour rotating pseudonyms = SESSION_{bank}_{hash}_{date}.
from core.crypto.session_id import SessionIDGenerator
# [DOC] BlockchainBlock: the in-memory block structure used during PoW mining (distinct from the DB model).
from core.blockchain.public_chain.block import Block as BlockchainBlock
# [DOC] Blockchain: manages the in-memory chain of blocks with PoW validation.
